...
The system SHALL provide an `AIService` class that wraps LLM provider calls. All AI features SHALL call `AIService.generate(prompt, user_id, context)` instead of calling providers directly. The service SHALL support swapping providers via configuration without code changes. The `context` parameter is a dict that carries feature-specific data (course info, task info, etc.) and defaults to None.
...
- **THEN** service reads config from ai_config table, checks budget, calls provider, logs the call to `ai_usage_logs`, returns generated text
...
- **THEN** subsequent AI calls use the new configuration on the next call (no restart required), because config is read from ai_config table per call

### Requirement: AI usage logging
The system SHALL log every AI call to the `ai_usage_logs` table with: user_id, endpoint (which feature triggered it), model, prompt_tokens, completion_tokens, cost_microdollars (estimated cost in microdollars as INTEGER), latency_ms, status (success/error), created_at.
...
- **THEN** cost_microdollars = (500 * 0.15 + 200 * 0.60) / 1_000_000 * 1_000_000 = 195 (stored as INTEGER)
...
### Requirement: Admin can test AI connectivity
The system SHALL provide an admin endpoint to trigger a single AI call to verify the full stack works: config loading, budget check, provider call, usage logging, and error handling.

**User story:** As an admin, I want to verify that the AI integration works end-to-end without needing to use a feature-specific flow.

#### Scenario: Successful test call
- **WHEN** admin submits POST /api/ai/test
- **THEN** system makes a real AI call with a test prompt, returns the generated text and the usage log id

#### Scenario: Test call with invalid config
- **WHEN** admin submits POST /api/ai/test but AI_API_KEY is not configured
- **THEN** system returns 503 with message "AI 服务未配置"

### Requirement: Admin can manage AI configuration
The system SHALL provide admin endpoints for reading and updating AI settings: model name, per-role token budgets, and API key status indicator. Changes take effect on the next AI call.

**User story:** As an admin, I want to adjust AI settings without SSH access to the server.

#### Scenario: Read current config
- **WHEN** admin submits GET /api/ai/config
- **THEN** system returns current model, per-role budgets, and whether API key is configured (key not returned, only is_configured boolean)

#### Scenario: Update model
- **WHEN** admin submits PUT /api/ai/config with model="gpt-4o"
- **THEN** system updates ai_config row, next AI call uses new model

#### Scenario: Update budgets
- **WHEN** admin submits PUT /api/ai/config with budgets={"admin":500000,"teacher":200000,"student":50000}
- **THEN** system updates ai_config rows, new budget limits apply immediately

#### Scenario: API key status
- **WHEN** admin views config and AI_API_KEY env var is set
- **THEN** is_configured=true. If not set, is_configured=false and all AI endpoints return 503.
